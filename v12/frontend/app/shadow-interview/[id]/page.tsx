...
import { 
  ...
  Play, Check, X, BarChart3, Loader2, Brain,
  StopCircle
} from "lucide-react";
...
export default function InterviewPage() {
      ...
      ws.onmessage = (msg) => {
        try {
          ...
          if (data.type === "user_transcript_delta") {
            ...
            // Don't show buffer errors to user - they're handled by server
            if (!data.message.includes('buffer too small')) {
  ...
  async function handleSubmitAnswer() {
    ...
    // ✅ FIX: Signal server before stopping
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'submit_answer' }));
    }
